...
namespace Kino
{
	...
	public:
		...
		inline const Vector operator+() const { return *this; }
		inline const Vector operator-() const { return Vector(-x, -y, -z); }

		inline const Vector operator+(const Vector & v) const { return Vector(x + v.x, y + v.y, z + v.z); }
		inline const Vector operator-(const Vector & v) const { return Vector(x - v.x, y - v.y, z - v.z); }

		inline const Vector & operator+=(const Vector & v) { x += v.x, y += v.y, z += v.z; return *this; }
		inline const Vector & operator-=(const Vector & v) { x -= v.x, y -= v.y, z -= v.z; return *this; }

		inline const Vector operator*(Element m) const { return Vector(x * m, y * m, z * m); }
		inline const Vector & operator*=(Element m) { x *= m, y *= m, z *= m; return *this; }
		...
		inline Element squaredLength() const { return            x * x + y * y + z * z;  }
		inline Element length() const        { return       sqrt(x * x + y * y + z * z); }
		inline Element inverseLength() const { return 1.0 / sqrt(x * x + y * y + z * z); }
