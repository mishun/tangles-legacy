...
{
	auto embedding = calculateEmbedding(graph, depth, relax);
...
std::vector<Geometry::Chain2D> CircleDrawer::calculateEmbedding(const Topology::EmbeddedGraph & graph, size_t depth, bool relax)
{
	assert(graph.genus() == 0);

	if(relax)
		return relaxEmbedding(graph, derivativeEmbedding(graph, depth));
	else
		return derivativeEmbedding(graph, depth);
}
...
namespace Draw
{
	...
	{
		const double k_border = 3.0;
		const double k_electr = 0.24;
		const double k_bend = 30.0;
