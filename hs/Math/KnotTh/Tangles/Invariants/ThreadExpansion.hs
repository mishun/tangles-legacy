...
threadExpansion invariant tangle = List.sort $ map (processThreadSet invariant tangle) sets
	where
...
processThreadSet invariant tangle threads = (ecode, invariant (threadTangle, circles))
	where
		targetLegs = List.sort $ List.concatMap (\ t -> let a = fst $ head t in if isLeg a then [a, snd $ last t] else []) threads

		targets = List.sort $ snd $ foldl (\ s t -> foldl checkTarget s t) (Set.empty, []) threads
			where
		...
		indices = Array.array (crossingsRange tangle) $ (zip (allCrossings tangle) (List.repeat 0)) ++ (zip targets [1 ..])

		findTarget u
			| isLeg v    =
				case List.elemIndex v targetLegs of
					Just i  -> (0, i)
		...
		circles = length $ filter (List.all (\ (_, d) -> (indices Array.! (incidentCrossing d)) == 0)) $ filter (not . isLeg . fst . head) threads

		ecode = List.sort $ map (\ t -> dist (fst $ head t) (snd $ last t)) $ filter (isLeg . fst . head) threads
			where
