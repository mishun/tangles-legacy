...
contextToMP context = List.intercalate " " $ filter (not . null) [dash, width, color]
	where
		...
		dash = case (fromJust $ dashType context) of
			DashedSolid  -> ""
			DashedEvenly -> printf "dashed dashpattern (on %.4f off %.4f)" width width

			where
				width = 4 * (maybe 1.0 fst (drawLineWidth context))
